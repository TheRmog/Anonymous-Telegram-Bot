    ...
    if path == HOOK:
        try:
            ...
            if "callback_query" in tgResponse:
                ...
                await db.prepare("update users set target_user = ? WHERE telegram_user_id = ?").bind(
                    targetReply.telegram_user_id, str(chatId)).run()
            ...
            if "message" in tgResponse:
                ...
                if "text" in message and message["text"].startswith("/start"):
                    ...
                    startedUser = await db.prepare("SELECT * FROM users WHERE telegram_user_id = ?").bind(
                        str(chatId)).first()
                    if startedUser:
                        startedUserId = startedUser.id
                    else:
                        startedUser = await db.prepare(
                            "INSERT INTO users (telegram_user_id, rkey, target_user) VALUES (?, ?, ?)").bind(
                            str(chatId), rndKey(), "").run()
                    ...
                    if match:
                        ...
                        targetUser = await db.prepare("SELECT * FROM users WHERE id = ? and rkey = ?").bind(
                            revHxId(param_id), param_rkey).first()
                        ...
                        if targetUser:
                            ...
                            await db.prepare("update users set target_user = ? WHERE id = ?").bind(
                                targetUser.telegram_user_id, startedUserId).run()
                            await postReq("sendMessage", {
                                "chat_id": chatId,
                                "text": "در حال ارسال پیام ناشناس به " + str(getChatMember["result"]["user"][
